 ...
 * socket read until the TCP stack gives up. That is why the pool below is cached and unbounded
 * rather than a fixed thread or two — a stuck call must not be able to hold up the calls after
 * it — and why its threads are daemons, so a hung target cannot keep this process alive at
 * shutdown.
 */
public final class JvmTargetHandle implements AutoCloseable {
     ...
     */
    synchronized MBeanServerConnection connection() throws IOException {
        if (jmxUrl == null) {
    ...
    <T> T call(Duration timeout, JmxCall<T> body) throws Exception {
        ...
        Future<T> future = calls.submit((Callable<T>) () -> body.run(connection()));
        try {
